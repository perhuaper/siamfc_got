...
class SiamVGG(nn.Module):
    ...
    def __init__(self):
        ...
        for i in range(len(self.feature.state_dict().items()) - 2):
            list(self.feature.state_dict().items())[i][1].data[:] = list(mod.state_dict().items())[i][1].data[:]
...
class TrackerSiamVGG(Tracker):

    def __init__(self, net_path=None, **kargs):
        super(TrackerSiamVGG, self).__init__(
            name='SiamVGG', is_deterministic=True)
        self.cfg = self.parse_args(**kargs)
    ...
    def update(self, image):
        ...
        with torch.set_grad_enabled(False):
            ...
            instances = self.net.feature(instance_images)
            responses = F.conv2d(instances, self.kernel) #* 0.001
            responses = self.net.bn_adjust(responses)
        responses = responses.squeeze(1).cpu().numpy()
